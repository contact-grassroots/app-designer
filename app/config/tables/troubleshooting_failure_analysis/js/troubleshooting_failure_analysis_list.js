...
var listQuery = 'SELECT *, strftime(\'%d-%m-%Y\', troubleshooting_failure_analysis.tfa_date) AS formatted_tfa_date ' +
    'FROM troubleshooting_failure_analysis ' +
    'JOIN refrigerators ON refrigerators._id = troubleshooting_failure_analysis.refrigerator_id ' +
    'WHERE troubleshooting_failure_analysis._sync_state != ?';

var listQueryParams = [util.deletedSyncState];
var searchParams = '(troubleshooting_failure_analysis.tfa_date LIKE ?)';
...
function resumeFunc(state) {
    if (state === 'init') {
        ...
        listViewLogic.setTableId('troubleshooting_failure_analysis');
        ...
        listViewLogic.showEditAndDeleteButtons(true, 'troubleshooting_failure_analysis');

		var dateTFATxt = odkCommon.localizeText(locale, "date_no_colon");
		var refIDTxt = odkCommon.localizeText(locale, "refrigerator_id_no_colon");
		
		// Use column IDs as-is here
		listViewLogic.setColIdsToDisplayInList(dateTFATxt, 'formatted_tfa_date', 
